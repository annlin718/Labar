...
var Deck = (function () {	
  ...
  function _card(i) {
    ...
    self.animateTo = function (params) {
	  ...
	  var _params$opacity = params.opacity;
      var opacity = _params$opacity === undefined ? self.opacity : _params$opacity;
  ...
  var shuffleEND = {	  
    deck: function deck(_deck3) {
      _deck3.shuffleEND = _deck3.queued(shuffleEND);
	  
      function shuffleEND(next) {
		  /*document.getElementById("container").*/
		  $("#container").animate({opacity:"0"},500);
		  $("#divBtn").animate({opacity:"1"},500);
  ...
  function Deck(jokers) {
	...
	var cards = new Array(jokers ? 20 : 20); //ANN_更改牌張數!!
  ...
  Deck.ease = ease;
  Deck.modules = { bysuit: bysuit, fan: fan, intro: intro, poker: poker, shuffle: shuffle, sort: sort, flip: flip,shuffleEND: shuffleEND };
  Deck.Card = _card;
  Deck.prefix = prefix;
